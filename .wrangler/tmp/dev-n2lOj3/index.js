...
var ChatRoom = class {
  ...
  async fetch(req) {
    ...
    server.accept();
    const broadcastPresence = /* @__PURE__ */ __name(() => {
    ...
    server.addEventListener("message", async (ev) => {
      try {
        ...
        const schema = external_exports.object({
          kind: external_exports.enum(["text", "emoji", "image", "video", "ai"]).default("text"),
          room: external_exports.string().default("public"),
        ...
        if (m.kind === "ai") {
          try {
            ...
            const text = aiRes.response ?? aiRes.text ?? JSON.stringify(aiRes);
            const idAi = nanoid();
            await this.env.DB.prepare(
              "INSERT INTO messages (id, from_user_id, to_user_id, room, content_type, content, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)"
            ).bind(idAi, "ai", session.userId, m.room, "ai", text, (/* @__PURE__ */ new Date()).toISOString()).run();
            server.send(JSON.stringify({ type: "message", id: idAi, from: "AI", to: session.username, room: m.room, kind: "ai", content: text, created_at: (/* @__PURE__ */ new Date()).toISOString() }));
          } catch (e) {
  ...
  async lookupUserId(username) {
    const row = await this.env.DB.prepare("SELECT id FROM users WHERE username = ?1").bind(username).first();
...
var registerSchema = external_exports.object({ username: external_exports.string().min(3).max(32), password: external_exports.string().min(6).max(100) });
...
app.post("/api/register", async (c) => {
  ...
  try {
    ;
    ({ username, password } = registerSchema.parse(raw2));
  } catch (e) {
  ...
  await c.env.DB.prepare("INSERT INTO users (id, username, password_hash, created_at) VALUES (?1, ?2, ?3, ?4)").bind(id, username, hash, (/* @__PURE__ */ new Date()).toISOString()).run();
...
app.post("/api/login", async (c) => {
  ...
  try {
    ;
    ({ username, password } = loginSchema.parse(raw2));
  } catch (e) {
  ...
  const token = nanoid();
  const session = { userId: user.id, username, issuedAt: Date.now() };
  await c.env.SESSIONS_KV.put(`session:${token}`, JSON.stringify(session), { expirationTtl: 60 * 60 * 24 * 7 });
  return c.json({ token });
});
app.use("/api/*", async (c, next) => {
  ...
  const raw2 = await c.env.SESSIONS_KV.get(`session:${token}`);
  if (!raw2) return c.json({ error: "\u4F1A\u8BDD\u8FC7\u671F" }, 401);
  c.set("session", JSON.parse(raw2));
  await next();
});
app.get("/api/me", (c) => {
...
app.post("/api/me/password", async (c) => {
  const schema = external_exports.object({ currentPassword: external_exports.string().min(6, "\u5F53\u524D\u5BC6\u7801\u957F\u5EA6\u4E0D\u8DB3"), newPassword: external_exports.string().min(6, "\u65B0\u5BC6\u7801\u957F\u5EA6\u4E0D\u8DB3") });
  const raw2 = await c.req.json().catch(() => ({}));
  let body;
  try {
    body = schema.parse(raw2);
  } catch (e) {
...
app.get("/api/me/messages", async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT m.id, u.username as from_username, v.username as to_username, m.room, m.content_type, m.content, m.created_at
     FROM messages m
     LEFT JOIN users u ON u.id = m.from_user_id
     LEFT JOIN users v ON v.id = m.to_user_id
     WHERE (m.to_user_id = ?1 OR m.from_user_id = ?1) OR (m.to_user_id IS NULL AND m.room = 'public')
...
app.post("/api/upload", async (c) => {
  ...
  await c.env.MEDIA.put(key, await file.stream(), { httpMetadata: { contentType: file.type || void 0 } });
  const svcUrl = c.env.MEDIA_SVC_URL;
  let meta = null;
  if (svcUrl) {
    try {
      const signed = new URL(`/media/${encodeURIComponent(key)}`, c.req.url).toString();
      const r = await fetch(`${svcUrl}/probe`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ url: signed }) });
      if (r.ok) meta = await r.json();
    } catch {
    }
  }
  return c.json({ key, kind, meta });
});
...
app.post("/api/ai/chat", async (c) => {
  const schema = external_exports.object({ prompt: external_exports.string().optional(), messages: external_exports.array(external_exports.object({ role: external_exports.enum(["system", "user", "assistant"]), content: external_exports.string() })).optional() });
  const body = schema.parse(await c.req.json());
  const messages = body.messages ?? [{ role: "user", content: body.prompt ?? "" }];
  const resp = await c.env.AI.run("@cf/meta/llama-3.1-8b-instruct", { messages });
...
// .wrangler/tmp/bundle-K16V1Q/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-K16V1Q/middleware-loader.entry.ts
